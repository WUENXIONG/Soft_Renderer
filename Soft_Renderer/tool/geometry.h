...
template<size_t LEN,size_t DIM, typename T> vec<LEN,T> proj(const vec<DIM,T> &v) {
    vec<LEN,T> ret;
    for (size_t i=LEN; i--; ret[i]=v[i]);//i=3,然后3>0为真，并且执行--操作变为2，ret[2] = v[2],类推
    return ret;
}
...
template<size_t DimCols, typename T> vec<DimCols, T> operator*(const float a, const vec<DimCols, T>& rhs) {
    vec<DimCols, T> ret;
    for (size_t i = DimCols; i--; ret[i] =a * rhs[i]);
    return ret;
}
